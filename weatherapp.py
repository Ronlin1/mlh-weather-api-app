...
base = Tk()
base.title("MLH Global Hack Week Weather API APP")
...
def close_app():
    closeapp = tkinter.messagebox.askyesno("MLH Weather App", "Do you want to exit App?")
    if closeapp > 0:
        base.destroy()
        return
...
def credits_func():
    credits_func = tkinter.messagebox.showinfo(title="Credits", message='''Made with ♥ by
***ATUHAIRE- AFRO BOY
